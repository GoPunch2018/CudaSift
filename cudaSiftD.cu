...
#define H2 (SCALEDOWN_H/2)
    ...
    if (xp < (width + 4) && yp < (height + 4)) {
        ...
        brows[BW * ty + tx] =
                k0 * (v + ShiftDown(v, 4)) + k1 * (ShiftDown(v, 1) + ShiftDown(v, 3)) + k2 * ShiftDown(v, 2);
    }
    __syncthreads();
    ...
    if (tx < W2 && ty < H2 && xs < (width / 2) && ys < (height / 2)) {
        ...
        d_Result[ys * newpitch + xs] =
                k0 * (ptr[0] + ptr[4 * BW]) + k1 * (ptr[1 * BW] + ptr[3 * BW]) + k2 * ptr[2 * BW];
    }
}
...
#define H2 (SCALEDOWN_H/2)
    ...
    if (tx < W2 && ty < H2 && xs < (width / 2) && ys < (height / 2)) {
        ...
        d_Result[ys * newpitch + xs] =
                k0 * (ptr[0] + ptr[4 * W2]) + k1 * (ptr[1 * W2] + ptr[3 * W2]) + k2 * ptr[2 * W2];
    }
}
...
__global__ void
ExtractSiftDescriptorsCONSTNew(cudaTextureObject_t texObj, SiftPoint *d_sift, float subsampling, int octave) {
    __shared__ float gauss[16];
    __shared__ float buffer[128];
    __shared__ float sums[4];
...
__global__ void
ExtractSiftDescriptorsCONST(cudaTextureObject_t texObj, SiftPoint *d_sift, float subsampling, int octave) {
    __shared__ float gauss[16];
    __shared__ float buffer[128];
    __shared__ float sums[4];
...
__global__ void
ExtractSiftDescriptorsOld(cudaTextureObject_t texObj, SiftPoint *d_sift, int fstPts, float subsampling) {
    __shared__ float gauss[16];
    __shared__ float buffer[128];
...
__device__ void
ExtractSiftDescriptor(cudaTextureObject_t texObj, SiftPoint *d_sift, float subsampling, int octave, int bx) {
    __shared__ float gauss[16];
    __shared__ float buffer[128];
    __shared__ float sums[4];
...
__global__ void
FindPointsMultiTest(float *d_Data0, SiftPoint *d_Sift, int width, int pitch, int height, float subsampling,
...
__global__ void
FindPointsMultiNew(float *d_Data0, SiftPoint *d_Sift, int width, int pitch, int height, float subsampling,
                   float lowestScale, float thresh, float factor, float edgeLimit, int octave) {
#define MEMWID (MINMAX_W + 2)
...
__global__ void FindPointsMulti(float *d_Data0, SiftPoint *d_Sift, int width, int pitch, int height, float subsampling,
                                float lowestScale, float thresh, float factor, float edgeLimit, int octave) {
#define MEMWID (MINMAX_W + 2)
    __shared__ unsigned int cnt;
    __shared__ unsigned short points[3 * MEMWID];
...
__global__ void
FindPointsMultiOld(float *d_Data0, SiftPoint *d_Sift, int width, int pitch, int height, float subsampling,
                   float lowestScale, float thresh, float factor, float edgeLimit, int octave) {
#define MEMWID (MINMAX_W + 2)
...
__global__ void
LaplaceMultiTex(cudaTextureObject_t texObj, float *d_Result, int width, int pitch, int height, int octave) {
...
__global__ void LowPass(float *d_Image, float *d_Result, int width, int pitch, int height) {
    ...
    if (tx < LOWPASS_W && xp < width && yp < height)
        d_Result[yp * pitch + xp] = kernel[4] * buff[tx + 4] +
                                    kernel[3] * (buff[tx + 3] + buff[tx + 5]) +
                                    kernel[2] * (buff[tx + 2] + buff[tx + 6]) +
                                    kernel[1] * (buff[tx + 1] + buff[tx + 7]) +
                                    kernel[0] * (buff[tx + 0] + buff[tx + 8]);
}
