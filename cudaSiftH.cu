...
void ExtractSift(SiftData &siftData, CudaImage &img, int numOctaves, double initBlur, float thresh, float edgeThreshold,
    ...
    if (!scaleUp) {
        ...
        TimerGPU timer1(0);
        ExtractSiftLoop(siftData, lowImg, numOctaves, 0.0f, thresh, edgeThreshold, lowestScale, 1.0f, memoryTmp,
        ...
        PrepareLaplaceKernels(numOctaves, 0.0f, kernel);
        safeCall(cudaMemcpyToSymbolAsync(d_LaplaceKernel, kernel, 8 * 12 * 16 * sizeof(float)));
        ExtractSiftLoop(siftData, lowImg, numOctaves, 0.0f, thresh, edgeThreshold, lowestScale * 2.0f, 1.0f, memoryTmp,
                        memorySub + height * iAlignUp(width, 128));
        safeCall(
                cudaMemcpy(&siftData.numPts, &d_PointCounterAddr[2 * numOctaves], sizeof(int), cudaMemcpyDeviceToHost));
        siftData.numPts = (siftData.numPts < siftData.maxPts ? siftData.numPts : siftData.maxPts);
...
int
ExtractSiftLoop(SiftData &siftData, CudaImage &img, int numOctaves, double initBlur, float thresh, float edgeThreshold,
    ...
    if (numOctaves > 1) {
        ...
        ExtractSiftLoop(siftData, subImg, numOctaves - 1, totInitBlur, thresh, edgeThreshold, lowestScale,
                        subsampling * 2.0f,
                        memoryTmp, memorySub + (h / 2) * p);
    }
    ExtractSiftOctave(siftData, img, numOctaves, thresh, edgeThreshold, lowestScale, subsampling, memoryTmp);
#ifdef VERBOSE
    double totTime = timer.read();
...
void
ExtractSiftOctave(SiftData &siftData, CudaImage &img, int octave, float thresh, float edgeThreshold, float lowestScale,
                  float subsampling,
                  float *memoryTmp) {
    const int nd = NUM_SCALES + 3;
#ifdef VERBOSE
  ...
  TimerGPU timer0;
    ...
    FindPointsMulti(diffImg, siftData, thresh, edgeThreshold, 1.0f / NUM_SCALES, lowestScale / subsampling, subsampling,
                    octave);
#ifdef VERBOSE
...
double
FindPointsMulti(CudaImage *sources, SiftData &siftData, float thresh, float edgeLimit, float factor, float lowestScale,
...
#else
    FindPointsMultiNew<<<blocks, threads>>>(sources->d_data, siftData.d_data, w, p, h, subsampling, lowestScale, thresh,
